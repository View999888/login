...
public class ImageUtil {
	...
	public static Bitmap getBitmap(String url) {
		...
		try {
			...
			com.wkx.utils.LogUtil.d(ImageUtil.class, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	public static Bitmap getBitmap(InputStream inputStream,int desiredWidth, int desiredHeight) {
		Bitmap bitmap = null;
		try {
			byte [] data = com.wkx.utils.StreamUtil.stream2bytes(inputStream);
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			com.wkx.utils.LogUtil.e(ImageUtil.class, "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			com.wkx.utils.LogUtil.e(ImageUtil.class, "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			com.wkx.utils.LogUtil.e(ImageUtil.class, "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					com.wkx.utils.LogUtil.d(ImageUtil.class,
	...
	public static String getHashCode(Bitmap bitmap) {
		...
		// 第三步，计算平均值。
		// 计算所有64个像素的灰度平均值。
		int avgPixel = com.wkx.utils.MathUtil.average(pixels);
		...
		for (int i = 0; i < comps.length; i += 4) {
			...
			hashCode.append(com.wkx.utils.MathUtil.binaryToHex(result));
		}
