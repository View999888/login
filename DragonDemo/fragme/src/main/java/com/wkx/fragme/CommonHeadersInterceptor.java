...
public class CommonHeadersInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!TextUtils.isEmpty(com.wkx.fragme.FrameApplication.getFrameApplication().getCookie())) {
            //headers(Headers.of(NetHeaders.getHeadMap()))//可通过map集合一次性添加多个请求头
            request = request.newBuilder().header("Cookie", com.wkx.fragme.FrameApplication.getFrameApplication().getCookie()).build();
        }
...
//        String session = headers.get("Set-Cookie");
        ...
        if (!TextUtils.isEmpty(cookies)) {
            com.wkx.fragme.FrameApplication.getFrameApplication().setCookie(cookies);
            SharedPrefrenceUtils.saveString(com.wkx.fragme.FrameApplication.getFrameApplicationContext(), ConstantKey.COOKIES, cookies);
        }
    }
}
